...
# TODO disruption data model change must be adapted since the exact cuts of depend on tmax !!!!
...
from custom_tools_max.miscellaneous import repeating_sequence
...
from custom_tools_max.output_handling import get_routes,plot_nodes_and_paths
...
parser = argparse.ArgumentParser(description="Process traffic scenario inputs.")
parser.add_argument("--file_id", type=str, required=True, help="Log File ID (integer)")
parser.add_argument("--traffic_scenario", type=float, required=True, help="Traffic Scenario (float, e.g., 0.1=sc.1, 0.6=sc.2)")
parser.add_argument("--lambda_mode", type=int, required=True, help="Lambda mode (integer: 0=none, 1=standard, 2=squared, 3=cubed)")
parser.add_argument("--solvertime", type=int, required=True, help="Solver time (integer)")
parser.add_argument("--runs", type=int, required=True, help="Model runs")
...
solvertime = args.solvertime
s = args.runs

import random
def generate_random_list(min_length=1, max_length=100, min_value=1, max_value=1000):
    list_length = 10
    random_list = set([random.randint(min_value, max_value) for _ in range(list_length)])
    random_list = sorted(random_list)
    return random_list

# file_id = input("Log File ID: int = ")
# traffic_scenario = float(input("Traffic Scenario (0.1=sc.1...0.2... 0.6=sc.2): float = "))
...
def log_experiment_result(result):
    try:
        file_path_kpi = f"exp_results/sol_2828_final_{file_id}.json"
...
def log_routes(routes_current_run,dataset,run,disruption_time):
    try:
        file_path_t_D_route = f"route_cache/routes_2828_final_{file_id}.json"
...
def main():
    ...
    def create_data_model_VRPTW():
        ...
        nodes, time_matrix, time_windows  = get_2828_instance()
        data = {}
        data["nodes_xy"] = nodes
        data['visited_nodes'] = []
        ...
        data["shift_duration"] = 480
        data["time_matrix"] = time_matrix
        data["time_windows"] = time_windows["tw(e_i,l_i)"].to_list()
        data["num_vehicles"] = 120
        data["depot_node_ids"] = repeating_sequence(5,data["num_vehicles"]/6)
        data["starts"] = repeating_sequence(5,data["num_vehicles"]/6)
        data["ends"] = repeating_sequence(5,data["num_vehicles"]/6)
    ...
    lambdas_squared = {"mode": "lambda^2 TD-DVRPTW","values":{key:value**2 for key,value in lambdas_standard["values"].items()}}
    lambdas_cubed = {"mode": "lambda^3 TD-DVRPTW","values":{key:value**3 for key,value in lambdas_standard["values"].items()}}
    ...
    for t_s in solver_times:
        ...
        data_model_VRPTW_og["lambdas"] = lambdas_options[lambda_mode]["values"]
        # =========================================================================
        # SOLVING INITIAL VRPTW
        # =========================================================================
        file_path_vrptw_sol = r"route_cache\routes_2828.json"
                            ...
                            for OVERTIME_PENALTY in overtimes:
                                for CUSTOMER_OUTSOURCE_PENTALY in customer_drops:
                                    for ADD_VEHICLE_COST in add_vehicles:
                                        ...
                                        for q_run in range(0,s):

                                            random_list = generate_random_list(5,10,20,450)

                                            print("-"*80)
                                            print(f"{total_runs+1}/{s*no_scenarios} in progress...")
                                            print("-"*80)
                                            ...
                                            max_simulations = (data_model_VRPTW["shift_duration"]*1.5)
                                            ...
                                            while simulation_period <= max_simulations:
                                                ...
                                                # GENERATE DEVIATIONS
                                                # =========================================================================
                                                # print(f"[Simulation] [{timestamp()}] create status_report...")
                                                try:
                                                    status_report = {
                                                ...
                                                # print(f"\n[Simulation] [{timestamp()}] update_time_matrix_dynamic...")
                                                ...
                                                # print(f"\n[Simulation] [{timestamp()}] update_time_matrix_dynamic done")        
                                                ...
                                                # print(f"\n[Simulation] [{timestamp()}] get_all_arcs_np done")
                                                all_route_arcs_tuples = [tuple(row) for row in all_route_arcs]
                                                # print(f"\n[Simulation] [{timestamp()}] all_route_arcs_tuples done")
                                                modified_arcs_tuples = [tuple(row) for row in modified_arcs]
                                                # print(f"\n[Simulation] [{timestamp()}] modified_arcs_tuples done")
                                                # Convert modified_arcs_tuples to a set for faster lookup
                                                modified_arcs_set = set(modified_arcs_tuples)

                                                # Use list comprehension to filter arcs
                                                affected_arcs = np.array([arc for arc in all_route_arcs_tuples if arc in modified_arcs_set])

                                                # affected_arcs = np.array([arc for arc in all_route_arcs_tuples if arc in modified_arcs_tuples])
                                                # Print the affected arcs
                                                # print(f"\n[Simulation] [{timestamp()}] affected_arcs done")
                                                print("\nAffected route arcs: ") if po else None
                                                print(affected_arcs) if po else None
                                                
                                                # if p <= 14.0:
                                                # print("\nFeasibility in Period = ",simulation_period)
                                                ...
                                                if simulation_period in random_list:
                                                # if potential_disruptions and simulation_period + 1 == min(list(potential_disruptions.values())):
                                                    ...
                                                    # if solution_ort:
                                                        ...
                                                        if solution_ort:
                                                            ...
                                                            print(f"[Simulation] (this should NEVER be the case LOL) edit could be init-sol misalignment")
